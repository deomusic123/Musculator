...
export function AppNav() {
  ...
  return (
    <>
      ...
      <nav className="fixed inset-x-4 bottom-4 z-40 flex items-center justify-between gap-2 rounded-[1.4rem] border border-[var(--border)] bg-[var(--surface)] p-2 shadow-[0_18px_50px_rgba(20,33,43,0.18)] backdrop-blur xl:hidden">
        {navItems.map((item) => {
          const active = isActive(pathname, item.href);
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex min-h-12 flex-1 items-center justify-center rounded-[1rem] px-3 py-2 text-sm font-medium transition ${
                active
